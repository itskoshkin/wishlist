...
func (s *ListStorageImpl) GetListBySharedLink(ctx context.Context, token string) (models.List, error) {
	var list models.List

	if err := s.pool.QueryRow(ctx, `SELECT id, user_id, image, title, notes, is_public, share_token, created_at, updated_at FROM lists WHERE share_token = $1`, token).Scan(
		&list.ID, &list.UserID, &list.Image, &list.Title, &list.Notes, &list.IsPublic, &list.ShareToken, &list.CreatedAt, &list.UpdatedAt,
	); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return models.List{}, fmt.Errorf("failed to get list with token '%s': not found", token)
		}
		return models.List{}, fmt.Errorf("failed to get list with token '%s': %w", token, err)
	}
...
func (s *ListStorageImpl) UpdateListByID(ctx context.Context, id uuid.UUID, req models.UpdateListRequest) error {
	var clauses []string
	var args []any
	var index = 1
	...
	if result, err := s.pool.Exec(ctx, fmt.Sprintf("UPDATE lists SET"+" %s WHERE id = $%d", strings.Join(clauses, ", "), index), args...); err != nil {
		return fmt.Errorf("failed to update list with ID '%s': %w", id, err)
	} else if result.RowsAffected() == 0 {
		return fmt.Errorf("failed to update list with ID '%s': not found", id)
	}

	return nil
}

func (s *ListStorageImpl) RotateSharedLink(ctx context.Context, id uuid.UUID, token string) error {
	if result, err := s.pool.Exec(ctx, "UPDATE lists SET share_token = $1, updated_at = now() WHERE id = $2", token, id); err != nil {
		return fmt.Errorf("failed to update share token for list with ID '%s': %w", id, err)
	} else if result.RowsAffected() == 0 {
		return fmt.Errorf("failed to update share token for list with ID '%s': not found", id)
	}

	return nil
}

func (s *ListStorageImpl) DeleteListByID(ctx context.Context, id uuid.UUID) error {
	if result, err := s.pool.Exec(ctx, "DELETE FROM lists WHERE id = $1", id); err != nil {
		return fmt.Errorf("failed to delete list with ID '%s': %w", id, err)
	} else if result.RowsAffected() == 0 {
		return fmt.Errorf("failed to delete list with ID '%s': not found", id)
	}

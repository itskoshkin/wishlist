...
func (s *WishStorageImpl) GetWishByID(ctx context.Context, id uuid.UUID) (models.Wish, error) {
	var wish models.Wish

	if err := s.pool.QueryRow(ctx, `SELECT id, list_id, image, title, notes, link, price, currency, reserved_by, created_at, updated_at FROM wishes WHERE id = $1`, id).Scan(
		...
		if errors.Is(err, pgx.ErrNoRows) {
			return models.Wish{}, fmt.Errorf("failed to get wish with ID '%s': not found", id)
		}
		return models.Wish{}, fmt.Errorf("failed to get wish with ID '%s': %w", id, err)
	}
...
func (s *WishStorageImpl) UpdateWishByID(ctx context.Context, id uuid.UUID, req models.UpdateWishRequest) error {
	var clauses []string
	var args []any
	var index = 1
	...
	if result, err := s.pool.Exec(ctx, fmt.Sprintf("UPDATE wishes SET"+" %s WHERE id = $%d", strings.Join(clauses, ", "), index), args...); err != nil {
		return fmt.Errorf("failed to update wish with ID '%s': %w", id, err)
	} else if result.RowsAffected() == 0 {
		return fmt.Errorf("failed to update wish with ID '%s': not found", id)
	}
...
func (s *WishStorageImpl) DeleteWishByID(ctx context.Context, id uuid.UUID) error {
	if result, err := s.pool.Exec(ctx, "DELETE FROM wishes WHERE id = $1", id); err != nil {
		return fmt.Errorf("failed to delete wish with ID '%s': %w", id, err)
	} else if result.RowsAffected() == 0 {
		return fmt.Errorf("failed to delete wish with ID '%s': not found", id)
	}

...
type WishService interface {
	...
	UpdateWish(ctx context.Context, wishID, userID uuid.UUID, req models.UpdateWishRequest) error
	...
	DeleteWish(ctx context.Context, wishID, userID uuid.UUID) error
}
...
func (ctrl *WishesController) UpdateWish(ctx *gin.Context) {
	...
	if err = ctrl.wishService.UpdateWish(ctx, wishID, userID, req); err != nil {
		var forbiddenError svcErr.ForbiddenError
		if errors.As(err, &forbiddenError) {
			apiModels.Error(ctx, http.StatusForbidden, err.Error())
			return
...
func (ctrl *WishesController) ReserveWish(ctx *gin.Context) {
	...
	if err = ctrl.wishService.ReserveWish(ctx, wishID, userID); err != nil {
...
func (ctrl *WishesController) ReleaseWish(ctx *gin.Context) {
	...
	if err = ctrl.wishService.ReleaseWish(ctx, wishID, userID); err != nil {
		apiModels.InternalError(ctx, err.Error())
		return
	}
...
func (ctrl *WishesController) DeleteWish(ctx *gin.Context) {
	...
	if err = ctrl.wishService.DeleteWish(ctx, wishID, userID); err != nil {
		var forbiddenError svcErr.ForbiddenError
		if errors.As(err, &forbiddenError) {
			apiModels.Error(ctx, http.StatusForbidden, err.Error())
			return
